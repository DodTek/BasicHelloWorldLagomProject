...
/**
  * This provides an event sourced behavior. It has a state, [[HelloExperimentState]], which
  ...
  */
object HelloExperimentBehavior {

  /**
    * Given a sharding [[EntityContext]] this function produces an Akka [[Behavior]] for the aggregate.
    */ 
  def create(entityContext: EntityContext[HelloExperimentCommand]): Behavior[HelloExperimentCommand] = {
    ...
    create(persistenceId)
      .withTagger(
        ...
        AkkaTaggerAdapter.fromLagom(entityContext, HelloExperimentEvent.Tag)
      )
  ...
  private[impl] def create(persistenceId: PersistenceId) = EventSourcedBehavior
      .withEnforcedReplies[HelloExperimentCommand, HelloExperimentEvent, HelloExperimentState](
        persistenceId = persistenceId,
        emptyState = HelloExperimentState.initial,
...
/**
  ...
  */
case class HelloExperimentState(message: String, timestamp: String) {
  def applyCommand(cmd: HelloExperimentCommand): ReplyEffect[HelloExperimentEvent, HelloExperimentState] =
    cmd match {
      case x: Hello              => onHello(x)
      case x: UseGreetingMessage => onGreetingMessageUpgrade(x)
    }

  def applyEvent(evt: HelloExperimentEvent): HelloExperimentState =
    evt match {
      case GreetingMessageChanged(msg) => updateMessage(msg)
    }
  private def onHello(cmd: Hello): ReplyEffect[HelloExperimentEvent, HelloExperimentState] =
    Effect.reply(cmd.replyTo)(Greeting(s"$message, ${cmd.name}!"))

  private def onGreetingMessageUpgrade(
    cmd: UseGreetingMessage
  ): ReplyEffect[HelloExperimentEvent, HelloExperimentState] =
    Effect
...
object HelloExperimentState {

  /**
    * The initial state. This is used if there is no snapshotted state to be found.
    */
  def initial: HelloExperimentState = HelloExperimentState("Hello", LocalDateTime.now.toString)
  ...
  /**
    ...
    */
  val typeKey = EntityTypeKey[HelloExperimentCommand]("HelloExperimentAggregate")
  ...
  /**
    ...
    */
  implicit val format: Format[HelloExperimentState] = Json.format
}

/**
  * This interface defines all the events that the HelloExperimentAggregate supports.
  */
sealed trait HelloExperimentEvent extends AggregateEvent[HelloExperimentEvent] {
  def aggregateTag: AggregateEventTag[HelloExperimentEvent] = HelloExperimentEvent.Tag
}

object HelloExperimentEvent {
  val Tag: AggregateEventTag[HelloExperimentEvent] = AggregateEventTag[HelloExperimentEvent]
}

/**
  * An event that represents a change in greeting message.
  */
case class GreetingMessageChanged(message: String) extends HelloExperimentEvent
...
/**
  ...
  */
trait HelloExperimentCommandSerializable

/**
  * This interface defines all the commands that the HelloExperimentAggregate supports.
  */
sealed trait HelloExperimentCommand
    extends HelloExperimentCommandSerializable
...
case class UseGreetingMessage(message: String, replyTo: ActorRef[Confirmation])
    extends HelloExperimentCommand
...
/**
  ...
  * application loader.
  */
object HelloExperimentSerializerRegistry extends JsonSerializerRegistry {
  override def serializers: Seq[JsonSerializer[_]] = Seq(
    // state and events can use play-json, but commands should use jackson because of ActorRef[T] (see application.conf)
    JsonSerializer[GreetingMessageChanged],
    JsonSerializer[HelloExperimentState],
